...
mapping_config = pd.read_excel('business_configuration.xlsx')
...
for i in files_location:
    ...
    for transaction_file in csv_files:
        ...
        try:
            ...
            if (('.csv' in transaction_file) and (transaction_file in mapping_config['File Name'])):
                ...
                file1.rename(columns = {current_config.loc[0,'Address'].upper():config['CustomerAddress']},inplace = True)    
                ...
                for suffix in suffixes:
                    ...
                    df[config['FirstName']] = df[config['FirstName']].str.replace(suffix,'',inplace = True)
                
                    df.loc[df[config['LastName']].str.contains(suffix),'SUFFIX'] = suffix 
                
                    df[config['LastName']] = df[config['LastName']].str.replace(suffix,'',inplace = True)
