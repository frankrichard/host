...
for i in files_location:
    ...
    try:
        ...
        if (config['CDMS_file1']) in os.listdir(i):
            ...
            # hash(df,df1,config['HASH_1_columns'].split(','))
            
            # hash(df,df2,config['HASH_2_columns'].split(','))
            
            # final_df = pd.concat([final_df,df2],axis = 1)
            
            # final_df.rename(columns = {'hash_'+''.join(config['HASH_2_columns'].split(',')):'HASH_2'},inplace = True)
        ...
        #     # response = requests.post(url = 'http://mr403s0332d.palawangroup.com:4200/fileUploadExternalApi',headers = {'X-AUTH-TOKEN':'eyJ1c2VybmFtZSI6InN5c3RlbSIsInRva2VuIjoiODRjOWZmNmQtZTllMy00MWUwLWI0MDctZmY5ZGQ5YjFmYWU4In0=','Content-Type':'application/json'},json = body)
            ...
            corporate_customers.to_csv(i.replace(config['replace_string'],config['replace_with'])+"//invalid//corporate_customers.csv",index  = False)
